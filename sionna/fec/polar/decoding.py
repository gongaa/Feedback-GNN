...
class PolarSCLDecoder(Layer):
    ...
    def __init__(self,
        ...
        self._info_pos = np.setdiff1d(np.arange(self._n), self._frozen_pos)
        self._llr_max = 300. # internal max LLR value (not very critical for SC)
        assert self._k==len(self._info_pos), "Internal error: invalid " \
                                             "info_pos generated."
    ...
    def _update_pm(self, ind_u, msg_uhat, msg_llr, msg_pm):
        ...
        return msg_pm
        mask1 = tf.logical_and(u_hat==0.0, llr<0)
        tf.print("u_hat", tf.shape(u_hat))
        tf.print("mask1", tf.shape(mask1))
        mask2 = tf.logical_or(tf.logical_and(u_hat==0.0, llr>0), tf.logical_and(u_hat==1.0, llr<0))
        llr_in = llr
        tf.tensor_scatter_nd_update(llr_in, tf.where(mask1), -llr[mask1])
        tf.tensor_scatter_nd_update(llr_in, tf.where(mask2), 0.0)
        tf.print("llr in", llr_in)
        msg_pm += llr_in
        return msg_pm
    ...
    def _cn_op(self, x, y):
        ...
        sign = tf.multiply(tf.math.sign(x), tf.math.sign(y))
        llr_abs = tf.minimum(tf.abs(x), tf.abs(y))
        llr_out = tf.multiply(sign, llr_abs)
        return llr_out
    ...
    def call(self, inputs):
        ...
        Args:
            inputs tf.float32: Tensor of shape `[...,n]` containing the
                channel LLR values (as logits). 
                or a tuple (tf.float32, bool) a tensor and a boolean flag return_list.
        ...
        if isinstance(inputs, tuple):
            input, return_list = inputs
        else:
            input = inputs
            return_list = 0

        tf.debugging.assert_type(input, self._output_dtype,
                                 "Invalid input dtype.")
        # internal calculations still in tf.float32
        input = tf.cast(input, tf.float32)

        # last dim must be of length n
        tf.debugging.assert_equal(tf.shape(input)[-1],
                                  self._n,
                                  "Last input dimension must be of length n.")

        # Reshape inputs to [-1, n]
        tf.debugging.assert_greater(tf.rank(input), 1)
        input_shape = input.shape
        new_shape = [-1, self._n]
        llr_ch = tf.reshape(input, new_shape)
        ...
        if return_list == 1:
            # msg_pm has shape [bs, 2*list_size]
            # msg_uhat has shape [bs, 2*list_size, log2(n)+1, n]
            c_hat = msg_uhat[:, :self._list_size, -1, :]
            u_hat = msg_uhat[:, :self._list_size, 0, :]
            return tf.concat([c_hat, u_hat], 0)

        if return_list == 2:
            cand_ind = tf.argmin(msg_pm, axis=-1)
            c_hat = tf.gather(msg_uhat[:, :, -1, :], cand_ind, axis=1, batch_dims=1)
            c_hat_reshape = tf.reshape(c_hat, (-1, self.n))
            u_hat = tf.gather(msg_uhat[:, :, 0, :], cand_ind, axis=1, batch_dims=1)
            u_hat_reshape = tf.reshape(u_hat, (-1, self.n))
            return tf.concat([c_hat_reshape, u_hat_reshape], 0)
