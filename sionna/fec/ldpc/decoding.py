...
class LDPCBPDecoder(Layer):
    ...
    def _cn_update_tanh(self, msg, syndrome=None):
    ...
    def _cn_update_phi(self, msg, syndrome=None):
        """Check node update function implementing the exact boxplus operation.
    ...
    def _cn_update_minsum(self, msg, syndrome=None):
        """ Check node update function implementing the min-sum approximation.
    ...
    def build(self, input_shape):
        ...
        if self._stateful or self._is_syndrome:
            assert(len(input_shape)==2), \
                "For stateful/syndrome decoding, a tuple of two inputs is expected."
    ...
    def call(self, inputs):
        ...
        elif self._is_syndrome:
            llr_ch, syndrome = inputs
            syndrome = tf.map_fn(lambda s: tf.math.pow(-1, s), tf.cast(syndrome, tf.int32))
            syndrome = tf.cast(syndrome, tf.float32)
        else:
            llr_ch = inputs
