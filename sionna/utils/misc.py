...
class BinarySource(Layer):
    ...
    def call(self, inputs):
        print("Binary source inputs:", inputs)
        if self._seed is not None:
...
def sim_ber(mc_fun,
    ...
    def _print_progress(is_final, rt, idx_snr, idx_it, header_text=None):
        ...
        if qldpc:
            print("{: >9} |{: >11} |{: >11} |{: >12} |{: >13} |{: >12} |{: >12} |{: >10}".format(*row_text), end=end_str)
        else:
    ...
    if qldpc:
        header_text = ["p", "Flagged", "BLER", "flag errors", 
                       "block errors", "num blocks",
                       "runtime [s]", "status"]
    ...
    try:
        ...
        for i in tf.range(num_points):
            ...
            for ii in tf.range(max_mc_iter):
                ...
                if qldpc:
                    # always compare against all zero
                    s_hat, l_hat = outputs[0], outputs[1]
                    bit_e = count_block_errors(tf.zeros_like(s_hat), s_hat) # actually flagged error
                    block_e = count_block_errors(tf.zeros_like(l_hat), l_hat)

                    bit_n = tf.size(s_hat[...,-1])
                    block_n = tf.size(l_hat[...,-1])
